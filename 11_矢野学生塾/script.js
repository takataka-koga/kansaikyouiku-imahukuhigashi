...
function addRow(tableId) {
    ...
    let inputId = tableId === 'notificationTable' ? 'notificationInput' : 'tuitionInput';
    ...
    saveButton.onclick = function() {
        saveRow(newRow, span, editInput, tableId);
    };
    ...
    saveData(tableId); // データを保存
    document.getElementById(inputId).value = ""; // 入力フィールドをクリア
}

function saveRow(row, span, editInput, tableId) {
...
function loadSavedData() {
    loadTableData('notificationTable');
    loadTableData('tuitionTable');
}

function loadTableData(tableId) {
    let tableBody = document.getElementById(tableId).querySelector('tbody');
    let savedData = localStorage.getItem(tableId);
    if (savedData) {
